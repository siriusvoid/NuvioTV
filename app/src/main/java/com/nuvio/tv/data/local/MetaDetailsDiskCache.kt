...
/** On-disk copy of a title's composed meta, so reopening it skips the loading skeleton. */
@Singleton
class MetaDetailsDiskCache @Inject constructor(
    ...
    companion object {
        ...
        /** File-name prefix from id and type alone, so home can check for a copy without the full key. */
        ...
        /** Only a finished status skips the network; an unknown one counts as airing. */
    ...
    /** Titles with a stored copy, in memory so home can check each focused poster without disk IO. */
    ...
    /** Every setting that shapes the meta is in the key; the source addon isn't, so one copy serves every row. */
    fun keyFor(
        itemId: String,
        itemType: String,
    ...
    /** Any stored copy, fresh or not: the detail screen refreshes stale ones itself. */
        ...
        mutex.withLock {
            ...
            directory().listFiles()?.forEach { file ->
                ...
                if (prefix.length == PREFIX_LENGTH) {
                    ...
                    // Legacy file without a prefix; nothing will ever match it.
                    file.delete()
                }
            }
            indexLoaded = true
    ...
    /** Fire-and-forget so non-suspending cache clears can call it. */
    fun clearAsync() {
        clearScope.launch {
            mutex.withLock {
    ...
    private fun tmdbSignature(settings: TmdbSettings): String {
        ...
        // Only groups stored on the meta; recommendations and collections would fragment the cache.
        val flags = listOf(
            settings.useArtwork,
